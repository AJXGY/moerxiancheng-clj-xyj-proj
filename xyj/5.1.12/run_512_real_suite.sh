...
python3 - <<'PY' "${TARGET_ARTIFACT}" "${ARTIFACT_515}" "${MODEL_PATH}" "${DEVICE_TYPE}" "${SINGLE_DEVICE_IDS}" "${DUAL_DEVICE_IDS}" "${DRY_RUN}"
import json
import os
import sys
...
runtime_summary = {
    ...
    "dual_success": bool(dual.get("success", False)),
    "single_outputs_count": int(single.get("outputs_count", 0)),
    "dual_outputs_count": int(dual.get("outputs_count", 0)),
    "overall_pass": bool(single.get("success", False) and dual.get("success", False)),
}
