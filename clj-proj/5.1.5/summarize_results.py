...
def classify(preflight, single, dual):
    ...
    error_free = bool(single and not single["errors"]) and bool(dual and not dual["errors"])
    ...
    outputs_ready = bool(single and single["outputs_count"] > 0) and bool(dual and dual["outputs_count"] > 0)
    statuses.append(
        (
            "E",
...
def build_markdown(output_path, preflight, single, dual):
    ...
    sample_lines = []
    for label, payload in [("单卡", single), ("双卡", dual)]:
        if not payload:
            continue
    ...
    lines.extend(
        [
            ...
            f"- 双卡结果：`{os.path.dirname(output_path)}/dual/summary.json`",
            "",
            "## 关键校验说明",
            "",
...
def main():
    ...
    dual = load_json(os.path.join(args.artifacts_dir, "dual", "summary.json"))
